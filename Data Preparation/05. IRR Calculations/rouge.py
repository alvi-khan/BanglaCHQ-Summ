...
class Rouge:
    ...
    def clean_sample(self, sample):
        ...
        words = [" ".join(sentence.split()) for sentence in sample.split(".") if len(sentence) > 0]
        return " ".join(words)
    ...
    def get_scores(self, hyps, refs, avg=False, ignore_empty=False):
        ...
        hyps = [self.clean_sample(hyp) for hyp in hyps]
        refes = [self.clean_sample(ref) for ref in refs]
